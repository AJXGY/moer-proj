...
def topology_chart(model):
    ...
    body.append('<text x="72" y="158" font-size="14" font-family="Arial, Helvetica, sans-serif" fill="#334155">调度 / 请求管理 / 后处理</text>')
    gx = 320
...
def pipeline_chart(model):
    ...
    body.append('<text x="560" y="150" font-size="14" font-family="Arial, Helvetica, sans-serif" fill="#334155">数据并行: 1</text>')
    ...
    body.append('<text x="560" y="225" font-size="14" font-family="Arial, Helvetica, sans-serif" fill="#334155">连续批处理: 开启</text>')
    return wrap("多卡划分与并行方式", 340, "\n".join(body))


def continuous_batch_chart(model):
    schedule = model["microbatch_or_continuous_batch_logic"]["schedule"]
    body = []
    xs = [70, 250, 430, 610]
    for i, x in enumerate(xs):
        body.append(f'<rect x="{x}" y="110" width="130" height="60" rx="14" fill="#fde68a" stroke="#f59e0b"/>')
        body.append(f'<text x="{x+20}" y="145" font-size="18" font-family="Arial, Helvetica, sans-serif" fill="#92400e" font-weight="700">SLOT{i}</text>')
        if i < len(xs) - 1:
    ...
    for item in schedule:
        ...
        yy += 28
    return wrap("连续 Batch / Microbatch 执行逻辑", 360, "\n".join(body))
...
def dag_chart(model):
    ...
    pos = {
        ...
        "n8": (560, 260), "n9": (380, 320), "n10": (210, 320)
    }
    body = []
    for node in nodes:
        x, y = pos[node["id"]]
        ...
        color = "#dbeafe"
        if node["type"] in {"gpu_compute", "gpu_memory"}:
            color = "#dcfce7"
...
def main():
    ...
    write(os.path.join(CHARTS, "continuous_batch_logic.svg"), continuous_batch_chart(model))
    write(os.path.join(CHARTS, "dag_graph.svg"), dag_chart(model))
    write(os.path.join(ROOT, "5.1.12图表汇总.md"), index_md())
